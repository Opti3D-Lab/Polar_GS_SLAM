                               ...
                               float3* output_normal)
{
    ...
    glm::mat3 Vrk_inv;
	bool well_conditioned = Vrk_eigen_value[min_id]>0.00000001;  //用来判断矩阵是否奇异，可逆
	...
	if(isnan(uvh_mn.x)|| D==0)  // 如果是奇异矩阵，或者是nan，D存储的是特征值的个数或者状态
	{
		*output_normal = {0,0,0};
	}
	else
	...
	bool prefiltered)
{
	...
	float3 cov = computeCov2D(p_orig, focal_x, focal_y, tan_fovx, tan_fovy, cov3D, viewmatrix, normals + idx);  //  TODO:加idx是什么意思？
	...
	int * __restrict__ n_touched)
{
	...
	if (inside)
	{
        ...
        if(last_contributor)
        {
            ...
            len_normal = max(len_normal, NORMALIZE_EPS);    //TODO:NORMALIZE_EPS是什么？为什么原来就没定义？
