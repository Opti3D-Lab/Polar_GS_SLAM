 ...
 *
 * For inquiries contact  george.drettakis@inria.fr2
 */
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; j < min(BLOCK_SIZE, toDo); j++) {
            ...
            for (int ch = 0; ch < 3; ch++)
            {
                ...
                dL_dalpha += (c - accum_normal_rec[ch]) * dL_dchannel;
                //新增的法线相关计算会影响 dL_dalpha，使其包含法线的梯度贡献。
                //这使得 dL_dalpha 更加全面，能够反映更多因素对损失函数的影响。
                // Update the gradients w.r.t. normal of the Gaussian.
                // Store in shared memory first.
                if (ch == 0) {
                    dL_dnormals_shared[tid].x = skip ? 0.f : dchannel_dcolor * dL_dchannel;
                } else if (ch == 1) {
                    dL_dnormals_shared[tid].y = skip ? 0.f : dchannel_dcolor * dL_dchannel;
                } else if (ch == 2) {
                    dL_dnormals_shared[tid].z = skip ? 0.f : dchannel_dcolor * dL_dchannel;
                }
            }
			...
			if (tid == 0) {
                ...
                atomicAdd(&dL_dnormals[global_id * 3 + 0], dL_dnormals_acc.x);
                atomicAdd(&dL_dnormals[global_id * 3 + 1], dL_dnormals_acc.y);
                atomicAdd(&dL_dnormals[global_id * 3 + 2], dL_dnormals_acc.z);
			}
		}
	}
}
