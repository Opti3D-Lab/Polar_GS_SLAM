...
class SLAM_GUI:
    ...
    def add_camera(self, camera, name, color=[0, 1, 0], gt=False, size=0.01):   # size控制视锥体的大小
        W2C = (
            camera.T_gt.clone()
            if gt
            else camera.T.clone()
        ...
        if name not in self.frustum_dict.keys():
            ...
            frustum_material.line_width = 3.0  # 调整为所需的线宽

            # 使用新材质添加到场景
            self.widget3d.scene.add_geometry(name, frustum.line_set, frustum_material)
        else:
    ...
    def add_keyframe_connection(self, prev_id, current_id):
        """添加关键帧之间的绿色连接线"""
        name_prev = "keyframe_{}".format(prev_id)
        name_current = "keyframe_{}".format(current_id)

        # 确保两个关键帧都存在
        if name_prev not in self.frustum_dict or name_current not in self.frustum_dict:
            return

        # 获取两个关键帧的位置
        point_prev = self.frustum_dict[name_prev].view_dir[1]
        point_current = self.frustum_dict[name_current].view_dir[1]

        # 创建连接线
        points = [point_prev, point_current]
        lines = [[0, 1]]
        colors = [[0, 1, 0]]
        ...
        line_material.line_width = 3.0  # 设置线宽为x像素

        # 使用新材质添加到场景
        connection_name = "kf_connection_{}_{}".format(prev_id, current_id)
        self.widget3d.scene.remove_geometry(connection_name)  # 先移除可能存在的线
        self.widget3d.scene.add_geometry(connection_name, line_set, line_material)
    ...
    def receive_data(self, q):
        ...
        # 临时线的名称（用于后续删除）
        temp_connection_name = "current_to_latest_keyframe"
        ...
        # 处理单个关键帧 - 先处理关键帧，因为当当前帧变成关键帧时需要移除临时连接线
        if gaussian_packet.keyframe is not None:
            ...
            # 添加当前关键帧ID到列表中
            if gaussian_packet.keyframe.uid not in self.keyframe_ids:
                self.keyframe_ids.append(gaussian_packet.keyframe.uid)

                # 如果不是第一个关键帧，则连接当前关键帧与前一个关键帧
                if len(self.keyframe_ids) > 1:
                    prev_id = self.keyframe_ids[-2]
                    current_id = self.keyframe_ids[-1]
                    self.add_keyframe_connection(prev_id, current_id)

                # 移除临时连接线，因为当前帧已经变成关键帧
                self.widget3d.scene.remove_geometry(temp_connection_name)
        ...
        if gaussian_packet.keyframes is not None:
            for keyframe in gaussian_packet.keyframes:
                ...
                # 添加当前关键帧ID到列表中
                if keyframe.uid not in self.keyframe_ids:
                    self.keyframe_ids.append(keyframe.uid)
        ...
        if gaussian_packet.current_frame is not None:
            ...
            # 将当前帧与最新的关键帧连接起来
            if self.keyframe_ids and len(self.keyframe_ids) > 0:
                latest_keyframe_id = self.keyframe_ids[-1]
                # 创建当前帧与最新关键帧的连接
                if "keyframe_{}".format(latest_keyframe_id) in self.frustum_dict:
                    current_point = frustum.view_dir[1]
                    keyframe_point = self.frustum_dict["keyframe_{}".format(latest_keyframe_id)].view_dir[1]

                    points = [current_point, keyframe_point]
                    lines = [[0, 1]]
                    colors = [[1, 0, 0]]  # 红色
                    ...
                    # 先移除可能存在的线，再添加新线
                    self.widget3d.scene.remove_geometry(temp_connection_name)
                    self.widget3d.scene.add_geometry(temp_connection_name, line_set, line_material)
