...
def render(
    ...
    # 决定用python计算颜色还是光栅化器计算颜色，前者可调试，后者更快
    shs = None
    colors_precomp = None
    ...
    if mask is not None:
        rendered_image, radii, depth, normal, opacity = rasterizer(
        ...
        rendered_image, radii, depth, normal, opacity, n_touched = rasterizer(
            means3D=means3D,
            means2D=means2D,
            shs=shs,
    ...
    return {
        ...
        "viewspace_points": screenspace_points, # 屏幕空间的点坐标
        "visibility_filter": radii > 0, # 可见性过滤器，标识哪些高斯点是可见的
        "radii": radii, # 高斯点在屏幕上的半径
        "depth": depth,
        "normal": normal,
        "opacity": opacity,
        "n_touched": n_touched, # 被处理的高斯点数量
    }
