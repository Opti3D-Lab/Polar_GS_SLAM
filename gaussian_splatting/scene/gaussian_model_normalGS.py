...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):    # 初始化各种高斯属性
        self.active_sh_degree = sh_degree   # 原始为0
        print("正在使用的SH阶数: ", self.active_sh_degree)
    ...
    def build_covariance_from_scaling_rotation( # 根据缩放和旋转矩阵构建协方差矩阵
    ...
    def oneupSHdegree(self):    # 实际这个项目里没用
        if self.active_sh_degree < self.max_sh_degree:
            self.active_sh_degree += 1

    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):    # 从图像创建点云数据的预处理，负责提供或处理深度
        cam = cam_info
        # 曝光调整
        ...
        return self.create_pcd_from_image_and_depth(cam, rgb, depth, init)  # 具体干活的是这个
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        self.ply_input = pcd
        # 将点云数据转换为张量，并计算特征、尺度、旋转和不透明度。
        fused_point_cloud = torch.from_numpy(np.asarray(pcd.points)).float().cuda()
        fused_color = RGB2SH(torch.from_numpy(np.asarray(pcd.colors)).float().cuda())   # 使用了SH来表示颜色
        features = (
        ...
        normal[..., 2] = 1.0    # 将其 z 轴方向的分量设置为 1.0
        return fused_point_cloud, features, scales, rots, opacities, normal # 返回点云坐标、特征、尺度、旋转和不透明度

    def init_lr(self, spatial_lr_scale):
        self.spatial_lr_scale = spatial_lr_scale

    def extend_from_pcd(    # 将点云数据的输出转为可训练的张量，添加到现有的高斯模型中
        self, fused_point_cloud, features, scales, rots, opacities, normal, kf_id
    ):
        ...
        new_normal = nn.Parameter(normal.requires_grad_(True))
    ...
    def extend_from_pcd_seq(    # 调用前面的函数生成点云数据，然后添加到高斯模型中
        self, cam_info, kf_id=-1, init=False, scale=2.0, depthmap=None
    ):
        fused_point_cloud, features, scales, rots, opacities, normal = (
            self.create_pcd_from_image(cam_info, init, scale=scale, depthmap=depthmap)
        )
        self.extend_from_pcd(
            fused_point_cloud, features, scales, rots, opacities, normal, kf_id
        )
    ...
    def training_setup(self, training_args):
        ...
        l = [
            ...
            {
                "params": [self._normal],
                "lr": training_args.normal_lr,  # TODO:需要添加单独的法线学习率到训练参数中
                "name": "normal",
            },
            {
    ...
    def save_ply(self, path):
        ...
        # 从模型中提取点坐标、法线、特征、不透明度、缩放和旋转等数据，并将它们转换为 NumPy 数组。
        xyz = self._xyz.detach().cpu().numpy()
        normals = np.zeros_like(xyz)    # 没有就填0
        f_dc = (
        ...
        attributes = np.concatenate(
            (xyz, normals, f_dc, f_rest, opacities, normal, scale, rotation), axis=1
        )
        elements[:] = list(map(tuple, attributes))
        # 创建 PLY 元素并保存为 PLY 文件
